...
def app(df, X, y):
    ...
    if st.checkbox("Age vs MonthlyIncome"):
        sns.color_palette("hot_r", as_cmap=True)
        ax=sns.scatterplot(x="Age",y="MonthlyIncome",data=df,hue='Age')
        st.pyplot()

    if st.checkbox("MonthlyRate vs MonthlyIncome"):
        sns.color_palette("hot_r", as_cmap=True)
        ax=sns.scatterplot(x="MonthlyRate",y="MonthlyIncome",data=df,hue='Age')
        st.pyplot()

    if st.checkbox("HourlyRate vs OverTime"):
        sns.color_palette("rocket", as_cmap=True)
        ax=sns.scatterplot(x="HourlyRate",y="OverTime",data=df,hue='JobRole')
        st.pyplot()
    
    if st.checkbox("EmployeeNumber vs Attrition"):
        sns.color_palette("winter", as_cmap=True)
        ax=sns.scatterplot(x="EmployeeNumber",y="Attrition",data=df)
        st.pyplot()
